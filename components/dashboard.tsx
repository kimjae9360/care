...
function DeviceChip({
  ...
  state: DeviceState
  label: string
  kind: "pillbox" | "wearable"
}) {
  const Icon = kind === "pillbox" ? PillBottle : Watch
  const name = kind === "pillbox" ? "스마트약통" : "웨어러블"
  const tone =
    state === "ok"
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-wrap gap-1.5">
                      <DeviceChip
                        state={p.devices.pillbox.state}
                        label={p.devices.pillbox.label}
                        kind="pillbox"
                      />
                      <DeviceChip
...
function SensorsSection() {
  ...
  const stateTone: Record<DeviceState, string> = {
      ...
      {patients.map((p) => (
        ...
        >
          ...
          <ul className="mt-4 flex flex-col gap-3">
            {(
              [
                { icon: PillBottle, name: "스마트 약통", d: p.devices.pillbox },
                { icon: Watch, name: "웨어러블 밴드", d: p.devices.wearable },
              ] as const
            ).map(({ icon: Icon, name, d }) => (
              <li
                key={name}
                className="flex items-center justify-between rounded-xl bg-muted/50 px-3.5 py-3"
              >
                <span className="flex items-center gap-2 text-sm font-medium text-card-foreground">
                  <Icon className="h-4 w-4 text-muted-foreground" aria-hidden />
                  {name}
                </span>
                <span className={`text-xs font-semibold ${stateTone[d.state]}`}>
                  {stateLabel[d.state]}
                </span>
              </li>
            ))}
